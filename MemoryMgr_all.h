...
inline T AddressByVersion(uintptr addressIII10, uintptr addressIII11, uintptr addressIIISteam, uintptr addressvc10, uintptr addressvc11, uintptr addressvcSteam, uintptr addressSA10)
{
	if(gtaversion == -1){
		...
		else if(*(uintptr*)0x5C2135 == 0xB85548EC) gtaversion = III_11;
		else if(*(uintptr*)0x5C6FD5 == 0xB85548EC) gtaversion = III_STEAM;
...
inline void ExtractCall(void *dst, uintptr a)
{
	*(uintptr*)dst = (uintptr)(*(uintptr*)(a+1) + a + 5);
}
template<typename T>
inline void InterceptCall(void *dst, T func, uintptr a)
{
	ExtractCall(dst, a);
	InjectHook(a, func);
}
template<typename T>
inline void InterceptVmethod(void *dst, T func, uintptr a)
{
	*(uintptr*)dst = *(uintptr*)a;
	Patch(a, func);
}
